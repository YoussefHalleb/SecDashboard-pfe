...
export default function Dashboard() {
      ...
      setRecommendations((prev) =>
        prev.map((r) => {
          if (r.finding_id === updated.finding_id) {
            if (r.id === updated.id) return { ...r, status: "approved", jira_pending: true };
  ...
  const reject = async (recId: string) => {
    try {
      ...
      const updated: Recommendation = res.data;
      setRecommendations((prev) =>
        prev.map((r) => (r.id === updated.id ? { ...r, status: "rejected" } : r))
      );
    } catch (e) {
      console.error(e);
      alert("Reject failed");
  ...
  const saveDeveloperPriority = async (finding: any, developerPriority: string) => {
    ...
    const scoreMap: Record<string, number> = {
      Critical: 95, High: 75, Medium: 50, Low: 25, "False Positive": 0, "Accepted Risk": 10,
    };
...
const loadAdaptiveStats = async (repoId: number) => {
  const res = await api.get(`/api/repositories/${repoId}/adaptive-ranking-stats`);
  setAdaptiveStats(res.data);
  return res.data;
};
const loadAiRanking = async (repoId: number) => {
  const res = await api.get(`/api/repositories/${repoId}/dataset-rank-findings`);
  ...
  const loadDeveloperRanking = async (repoId: number) => {
    ...
    items.forEach((f: any) => { if (f.developer_reason) reasons[f.id] = f.developer_reason; });
...
await loadAdaptiveStats(selectedRepo.id);
    ...
    try {
      ...
      const res = await api.post("/api/ai/recommendations", {
        ...
        vulnerabilities: limited.map((v) => ({
          id: v.id, title: v.title, severity: v.severity, scanner: v.scanner, description: v.description || "",
        })),
      });
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {data.map((repo: Repository) => {
            ...
            return (
              <div
                key={repo.id}
                className="bg-slate-900/95 border border-slate-800 rounded-2xl overflow-hidden hover:border-slate-700 transition group shadow-xl"
              >
                {/* Card accent line (like Login card top) */}
                <div className="h-[3px] bg-gradient-to-r from-emerald-500 via-blue-500 to-violet-500" />
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 shrink-0">
              ...
              <div className="flex items-center gap-2 flex-wrap justify-end">
                <div className="flex items-center gap-1.5">
                  <label className="text-xs text-slate-500 whitespace-nowrap">Jira Assignee ID</label>
                  <input
                    type="text"
  ...
  if (items.length > 0) {
    ...
    setLoadingRankRun(false);
                      ...
                      setTimeout(() => { clearInterval(poll); setLoadingRankRun(false); }, 60000);
                    } catch (e) { console.error(e); setLoadingRankRun(false); }
                  }}
                ...
                >
                  {loadingRankRun ? (
                    <><svg className="w-3 h-3 animate-spin" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z" /></svg>Ranking...</>
                  ) : <>🔄 Run AI Ranking</>}
                </button>
                <button
                ...
                <div className="bg-slate-800/40 border border-emerald-500/20 rounded-2xl overflow-hidden">
                  ...
                  <div className="p-4">
                    <div className="flex items-center justify-between mb-3">
                      <div>
                        <h3 className="text-sm font-bold text-emerald-400">🤖 Dataset-based Vertex AI Ranking</h3>
                        <p className="text-xs text-slate-500 mt-1">Trivy and ZAP ranked separately by the AI.</p>
                      </div>
                      <div className="flex items-center gap-2">
                        <button onClick={() => selectedRepo && loadAiRanking(selectedRepo.id)}
                        ...
                        <button onClick={() => selectedRepo && loadDeveloperRanking(selectedRepo.id)}
                          className="text-xs bg-blue-600 hover:bg-blue-500 text-white px-3 py-1.5 rounded-lg transition">
                          Dev order
                        </button>
                        <button onClick={saveDeveloperOrder}
                    ...
                    <div className="bg-slate-900/80 border border-slate-700/50 rounded-xl px-4 py-3 mb-4">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        {aiRankedFindings.some(f => f.developer_rank) ? (
                          <span className="text-xs bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 px-2 py-0.5 rounded-full">Active — dev corrections applied</span>
                        ) : (
                          <span className="text-xs bg-slate-800 border border-slate-700 text-slate-500 px-2 py-0.5 rounded-full">No corrections yet</span>
                        )}
                      </div>
                      <div className="space-y-1.5 text-xs text-slate-400">
                        {[
                          { color: "text-emerald-400", text: "AI ranks findings using CVSS, EPSS, KEV, evidence, and sensitive URLs by default." },
                          { color: "text-blue-400",    text: "When you reorder findings and save, the AI receives your corrections on the next ranking run." },
                          { color: "text-violet-400",  text: "The AI extracts patterns from your corrections — not just individual cases." },
                          { color: "text-orange-400",  text: "Your corrections always override the default rules when there is a conflict." },
                        ].map(({ color, text }, i) => (
  ...
  <div className="bg-slate-900/80 border border-violet-500/30 rounded-xl px-4 py-3 mb-4">
    ...
    <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
      <div className="bg-slate-800 border border-slate-700 rounded-lg p-2 text-center">
        ...
        <div className="text-xs text-slate-500">Trivy examples</div>
      </div>
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-lg p-2 text-center">
        ...
        <div className="text-xs text-slate-500">OWASP examples</div>
      </div>

      <div className="bg-slate-800 border border-slate-700 rounded-lg p-2 text-center">
        <div className="text-lg font-black text-emerald-400">
          {Number(adaptiveStats.trivy?.promoted || 0) + Number(adaptiveStats.owasp?.promoted || 0)}
        </div>
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-lg p-2 text-center">
        <div className="text-lg font-black text-red-400">
          {Number(adaptiveStats.trivy?.demoted || 0) + Number(adaptiveStats.owasp?.demoted || 0)}
        </div>
        <div className="text-xs text-slate-500">Demoted</div>
      </div>

      <div className="bg-slate-800 border border-slate-700 rounded-lg p-2 text-center">
        <div className="text-lg font-black text-blue-400">
          {Number(adaptiveStats.trivy?.accepted || 0) + Number(adaptiveStats.owasp?.accepted || 0)}
        </div>
    ...
    <p className="text-xs text-slate-500 mt-3">
      Vertex AI uses these scanner-specific feedback examples from Trivy and OWASP datasets during ranking.
    </p>
  </div>
)}
                    {/* Tabs ZAP / Trivy */}
                    <div className="flex gap-2 mb-4">
                      {(["zap", "trivy"] as const).map((tab) => {
                        const count = aiRankedFindings.filter((f) => normalizeScannerType(f) === tab).length;
                        return (
                          ...
                          >
                            {tab === "zap" ? "🌐 ZAP" : "🐳 Trivy"}
                            <span className="bg-slate-700 text-slate-300 px-1.5 py-0.5 rounded-full text-xs">{count}</span>
                          </button>
                        );
                      })}
                    </div>
                    ...
                    <div className="space-y-2">
                      {aiRankedFindings
                        ...
                        .map((f, index) => {
                          ...
                          const filteredLength = aiRankedFindings.filter((x) => normalizeScannerType(x) === rankingTab).length;
                          return (
                            <div key={f.id} className="bg-slate-900/80 border border-slate-700/60 rounded-xl px-4 py-3">
                              <div className="flex items-center justify-between gap-3">
                                <div className="flex items-center gap-3 min-w-0 flex-1">
                                  ...
                                  <div className="min-w-0">
                                    ...
                                    <div className="text-xs text-slate-500 mt-0.5">#{f.id} · {f.severity} · {f.scanner}</div>
                                  </div>
                                </div>
                                <div className="flex items-center gap-2 shrink-0">
                                  <span className={`text-xs font-bold px-2 py-0.5 rounded border ${
                                    f.ai_priority_label === "Critical" ? "bg-red-500/10 border-red-500/30 text-red-400"
                                    : f.ai_priority_label === "High"   ? "bg-orange-500/10 border-orange-500/30 text-orange-400"
                                    : f.ai_priority_label === "Medium" ? "bg-yellow-500/10 border-yellow-500/30 text-yellow-400"
                                    : "bg-emerald-500/10 border-emerald-500/30 text-emerald-400"
                                  }`}>{f.ai_priority_label}</span>
                                  <button onClick={() => moveFinding(globalIndex, "up")} disabled={index === 0}
                                    className="text-xs bg-slate-800 hover:bg-slate-700 border border-slate-700 disabled:opacity-30 text-slate-300 px-2 py-1 rounded-lg transition">↑</button>
                                  <button onClick={() => moveFinding(globalIndex, "down")} disabled={index === filteredLength - 1}
                                    className="text-xs bg-slate-800 hover:bg-slate-700 border border-slate-700 disabled:opacity-30 text-slate-300 px-2 py-1 rounded-lg transition">↓</button>
                                </div>
                              </div>
                              <p className="text-xs text-slate-400 mt-2">{f.ai_ranking_reason}</p>
                              <textarea rows={2} placeholder="Why did you reorder this? (optional)"
                                value={rankingReasons[f.id] ?? f.developer_reason ?? ""}
                                onChange={(e) => setRankingReasons((prev) => ({ ...prev, [f.id]: e.target.value }))}
                              ...
                              {f.developer_rank && (
                                <div className="flex items-center gap-2 mt-1 flex-wrap">
                                  <span className="text-xs text-blue-400">Developer rank: {f.developer_rank}</span>
                                  {f.developer_email && (
                                    <span className="text-xs bg-slate-800 border border-slate-700 text-slate-300 px-2 py-0.5 rounded-full">👤 {f.developer_email}</span>
                                  )}
                                </div>
                              )}
                            </div>
                      ...
                      {aiRankedFindings.filter((f) => normalizeScannerType(f) === rankingTab).length === 0 && (
                ...
                <div className="bg-slate-800/40 border border-purple-500/20 rounded-2xl overflow-hidden">
                  ...
                  <div className="p-4">
                    ...
                    <p className="text-xs text-slate-500 mb-4">
                      Initial score from AI/rules. Developer feedback will be stored and used to train the adaptive prioritization model.
                    </p>
                    <div className="space-y-2">
                      {Array.from(new Map(prioritizedFindings.map((f) => [`${f.title}-${f.scanner}`, f])).values())
                        .slice(0, 15)
                        .map((f) => (
                          <div key={f.id} className="bg-slate-900/80 border border-slate-700/60 rounded-xl px-4 py-3">
                            ...
                            {f.priority_reasons?.length > 0 && (
                              <div className="flex flex-wrap gap-1 mt-1">
                                {f.priority_reasons.map((r: string, i: number) => (
                                  <span key={i} className="text-xs bg-slate-800 border border-slate-700 text-slate-400 px-2 py-0.5 rounded-full">{r}</span>
                                ))}
                              </div>
                            )}
                            <div className="mt-3 pt-3 border-t border-slate-700/50">
                              <div className="flex items-center justify-between gap-3 mb-2">
                                ...
                                {f.developer_priority && (
                                  <span className="text-xs bg-blue-500/10 border border-blue-500/30 text-blue-400 px-2 py-0.5 rounded-full">Saved: {f.developer_priority}</span>
                                )}
                              </div>
                              <div className="flex flex-wrap gap-1.5">
                                {["Critical", "High", "Medium", "Low", "False Positive", "Accepted Risk"].map((priority) => (
                                  <button key={priority} disabled={savingFeedbackId === f.id}
                                    ...
                                    className={`text-xs font-semibold px-2 py-1 rounded-lg border transition disabled:opacity-50 ${
                                      f.developer_priority === priority
                                        ...
                                        : priority === "Critical" ? "bg-red-500/10 border-red-500/30 text-red-400 hover:bg-red-500/20"
                                        : priority === "High"     ? "bg-orange-500/10 border-orange-500/30 text-orange-400 hover:bg-orange-500/20"
                                        : priority === "Medium"   ? "bg-yellow-500/10 border-yellow-500/30 text-yellow-400 hover:bg-yellow-500/20"
                                        : priority === "Low"      ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/20"
              ...
              {!!aiMessage && (
                <div className={`flex items-center gap-2 text-xs px-4 py-2.5 rounded-xl border ${
                  ...
                  <span>{aiMessage}</span>
                  {!!aiSource && !loadingAI && (
                    <span className="ml-auto text-slate-500">{aiSource === "generated" ? "Generated now" : "From database"}</span>
                  )}
                </div>
              )}
                  ...
                  <div className="p-4">
                    ...
                    <div className="space-y-3">
                      {recommendations.map((rec) => (
                        <div key={rec.id} className="bg-slate-900/80 border border-slate-700/60 rounded-xl p-4">
                          <div className="flex items-start justify-between gap-3 mb-3">
                            <div>
                              <div className="font-bold text-sm text-violet-300 mb-1">{rec.title || "Vulnerability"}</div>
                            ...
                            {rec.false_positive_likelihood && (
                              <div className="text-center">
                                <div className={`text-sm font-bold ${
                                  ...
                                  : rec.false_positive_likelihood === "Medium" ? "text-yellow-400" : "text-red-400"
                                }`}>{rec.false_positive_likelihood}</div>
                          ...
                          {rec.code_fix_example && (
                            <div className="mt-3">
                              <div className="flex items-center justify-between mb-1">
                                ...
                                <button
                                  onClick={() => { if (rec.code_fix_example) { navigator.clipboard.writeText(rec.code_fix_example); alert("Copied!"); } }}
                          ...
                          <div className="flex items-center gap-2 pt-2 border-t border-slate-700/50 mt-3">
                            {rec.status === "approved" ? (
                              <div className="flex items-center gap-2 flex-wrap">
                                <span className="text-xs bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 px-2 py-1 rounded-lg">✓ Approved</span>
                                ...
                                {rec.jira_issue_key && rec.jira_issue_url && (
                                  <a href={rec.jira_issue_url} target="_blank" rel="noopener noreferrer"
                                ...
                                {rec.status === "approved" && !rec.jira_issue_key && !rec.jira_pending && (
                                  <button onClick={() => retryJira(rec.id)}
                              ...
                              <span className="text-xs bg-slate-800 border border-slate-700 text-slate-500 px-2 py-1 rounded-lg">Rejected</span>
                            ) : (
                              <>
                                {isAdmin && (
              ...
              {[
                { title: "⚡ ZAP",   color: "text-orange-400", scanner: "ZAP Scan",   accent: "from-orange-500 to-yellow-400" },
                { title: "🛡️ Trivy", color: "text-blue-400",   scanner: "Trivy Scan", accent: "from-blue-500 to-teal-400" },
              ].map(({ title, color, scanner, accent }) => {
                const vulns = selectedRepo.vulnerabilities.filter((v) => v.scanner === scanner);
                return (
                  <div key={scanner} className="bg-slate-800/40 border border-slate-700/50 rounded-2xl overflow-hidden">
                    <div className={`h-[2px] bg-gradient-to-r ${accent}`} />
                    <div className="p-4">
                      <div className="flex items-center gap-2 mb-3">
                        <h3 className={`font-bold text-sm ${color}`}>{title}</h3>
                        <span className="text-slate-500 text-xs bg-slate-800 border border-slate-700 px-2 py-0.5 rounded-full">{vulns.length}</span>
                      </div>
        ...
        <div className="fixed inset-0 bg-black/75 backdrop-blur-sm flex justify-center items-center z-50 p-4">
          <div className="bg-slate-900/98 border border-slate-800 w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[80vh] overflow-hidden">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 shrink-0">
              ...
              <button onClick={() => setShowUserManager(false)}
                className="w-8 h-8 flex items-center justify-center rounded-lg bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-400 hover:text-white transition text-lg">
                ×
              </button>
            </div>
            <div className="overflow-y-auto flex-1 p-6">
              {loadingUsers ? (
                ...
                <div className="space-y-3">
                  {users.map((u) => (
                    <div key={u.id} className="bg-slate-800/60 border border-slate-700/60 rounded-xl px-4 py-3 flex items-center justify-between gap-4">
                      <div className="min-w-0 flex-1">
                        <div className="text-sm font-semibold text-white truncate">{u.email}</div>
                        <div className="text-xs text-slate-500 mt-0.5">Créé le {new Date(u.created_at).toLocaleDateString()}</div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <span className={`text-xs font-bold px-2 py-0.5 rounded border ${
                          u.role === "admin"
                        ...
                        {u.id !== me?.id && (
                          <button onClick={() => changeUserRole(u.id, u.role === "admin" ? "developer" : "admin")}
                        ...
                        {u.id === me?.id && <span className="text-xs text-slate-500 px-2">vous</span>}
                      </div>
                    </div>
                  ))}
                </div>
...
function SeverityBadge({ severity }: { severity: string }) {
  ...
  return (
    <span className={`border text-xs font-semibold px-2.5 py-1 rounded-lg ${cls[severity] || "bg-slate-800 border-slate-700 text-slate-300"}`}>
      {severity}
    </span>
  );
}
